...
class Apply extends CI_Controller
{
	...
	function _status_intro($status = '')
	{
		...
		switch($status)
		{
			...
			case 'review_refused':
			case 'admit':
				$intro = '<p>您的申请已经成功导入到 iPlacard，我们将在近期内审核您提交的申请材料。审核通过时，将会有邮件通知。在此期间，请核对您的申请材料，如果内容有误，您将可以联系修改部分数据。</p>'
			...
			case 'interview_completed':
			case 'interview':
				$intro = '<p>在面试阶段中，我们将会根据您的委员会意向指派一位面试官，他将与您取得联系并且确定面试时间和面试方式。请尽量为面试预留足够的时间，如果您无法在约定的时间进行面试，请联系您的面试官，他将会重新为您安排面试时间。面试通过之后，您将可以收到席位分配结果。</p>'
					. '<p>如果面试没有通过，您将进入等待队列，当有面试通过的代表退会时，您将可以接替该代表的席位。</p>'
					. '<p>如果您对面试有任何疑问，请与您的面试官取得联系，同时您可以联系管理员，他将尽快回复您的问题。</p>';
				break;
			case 'seat_assigned':
			case 'seat':
				$intro = '<p>面试通过之后，面试官将会根据您的面试表现为您分配适合的席位。通常情况下，面试官将在完成面试后立即为您分配合适的席位。</p>'
					. '<p>席位分配后，您将可以在席位信息页面确认席位；如果您认为分配的席位不适合您，请联系您的面试官，他将可以为您重新分配席位。</p>';
				break;
			case 'invoice_issued':
			case 'payment_received':
			case 'pay':
