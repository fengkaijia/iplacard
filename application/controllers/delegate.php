...
class Delegate extends CI_Controller
{
	...
	function profile($uid)
	{
		...
		if($profile['status'] == 'deleted')
		{
			...
			$this->ui->alert(sprintf('管理员已经于%1$s计划删除了此帐户，此帐户数据将在%2$s删除。', date('Y年m月d日', $delete_time), nicetime(user_option('delete_time', time(), $uid) + option('delegate_delete_lock', 7) * 24 * 60 * 60, true)), 'danger');
		}
		...
		switch($action)
		{
			...
			case 'quit':
				...
				$this->email->html($this->parser->parse_string(option('email_delegate_quitted', "您已于 {time} 退会。\n\n"
						. "您的 iPlacard 帐号将于 {lock_time}（退会操作执行后 {lock_period} 天内）关闭，请立即登录 iPlacard 查看详情。如果这是管理员的误操作请立即联系管理员恢复帐户。"), $data, true));
				$this->email->send();
			...
			case 'delete_account':
				...
				$this->email->html($this->parser->parse_string(option('email_delegate_deleted', "管理员已经于 {time} 停用了您的 iPlacard 帐户。以下原因造成了账户停用：\n\n"
						. "\t{reason}\n\n"
						. "您的 iPlacard 帐号将于 {lock_time}（退会操作执行后 {lock_period} 天内）删除。请立即联系管理员了解情况。"), $data, true));
				$this->email->send();
